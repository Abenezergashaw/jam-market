      ...
      <div class="flex-1 min-w-0">
        ...
        <ul class="space-y-1">
          <li v-for="item in order.items" :key="item.id" class="flex items-center gap-2 text-xs text-zinc-500">
            ...
            <span class="text-zinc-400 ml-auto">${{ (Number(item.price) * item.quantity).toFixed(2) }}</span>
          </li>
        </ul>
      </div>

      <div class="flex flex-col items-end gap-3 shrink-0">
        <span class="text-base font-bold text-zinc-900">${{ Number(order.totalPrice).toFixed(2) }}</span>
        <div v-if="order.status === 'PENDING'" class="flex gap-2">
          <button class="btn-primary text-xs px-3 py-1.5" :disabled="loading" @click="$emit('confirm', order.id)">
            Confirm
          </button>
          <button class="btn-danger text-xs px-3 py-1.5" :disabled="loading" @click="$emit('cancel', order.id)">
            Cancel
          </button>
        </div>
      </div>
