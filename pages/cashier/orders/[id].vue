<template>
  <div class="space-y-5 max-w-4xl">
    <div class="flex items-center gap-2 text-sm">
      <NuxtLink to="/cashier/orders" class="text-zinc-400 hover:text-zinc-700 transition-colors">Orders</NuxtLink>
      <span class="text-zinc-300">/</span>
      <span class="text-zinc-700 font-semibold">#{{ route.params.id }}</span>
    </div>

    <div v-if="loading" class="space-y-4">
      <div class="card h-24 animate-pulse bg-zinc-100" />
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div class="card h-56 animate-pulse bg-zinc-100" />
        <div class="card h-56 animate-pulse bg-zinc-100" />
      </div>
    </div>
    ...
    <div v-else-if="!order" class="card p-10 text-center">
      ...
      <NuxtLink to="/cashier/orders" class="btn-secondary text-sm">Back to orders</NuxtLink>
    </div>
    ...
    <template v-else>
      ...
      <!-- Dispatch modal — delivery person picker -->
      <div v-if="showDispatchModal" class="card p-5 border-brand-200 bg-brand-50/50 space-y-3">
        <h3 class="text-sm font-semibold text-zinc-800">Assign Delivery Person</h3>
        <div v-if="deliveryPersons.length === 0" class="text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2">
          No delivery staff assigned to this store. Add delivery accounts in Admin → Users first.
        </div>
        <template v-else>
          <select v-model="selectedDeliveryPerson" class="input text-sm">
            <option :value="null" disabled>— Select delivery person —</option>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div class="card p-5">
          ...
          <div v-if="order.notes" class="border-t border-zinc-100 mt-4 pt-4">
            <p class="text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-1">Notes</p>
        ...
        <div class="card p-5 space-y-4">
          ...
          <!-- Payment actions -->
          <div v-if="order.paymentStatus === 'PENDING' && order.paymentMethod && order.paymentMethod !== 'CASH' && order.paymentMethod !== 'COD'" class="space-y-2 pt-1">
            <div>
              <label class="label text-xs">Note <span class="text-zinc-400 font-normal normal-case tracking-normal">(optional)</span></label>
              <input
                v-model="paymentNoteInput"
                type="text"
                maxlength="300"
                class="input text-sm"
                placeholder="e.g. Amount mismatch, reference unclear…"
        ...
        <div class="space-y-4">
          <div class="card p-5">
            ...
            <div class="space-y-2 text-sm">
              ...
              <div v-if="order.deliveryPersonId" class="flex items-start gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-zinc-400 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" />
                </svg>
                <span class="text-zinc-700">Assigned to delivery #{{ order.deliveryPersonId }}</span>
              </div>
            </div>
          </div>
...
const availableTransitions = computed(() => {
  ...
  const t = []
  if (s === 'PENDING') {
    if (perms.includes('orders:approve')) t.push({ to: 'CONFIRMED', label: 'Confirm Order', style: 'primary' })
    if (perms.includes('orders:cancel')) t.push({ to: 'CANCELLED', label: 'Cancel Order', style: 'danger' })
  } else if (s === 'CONFIRMED') {
    if (perms.includes('orders:dispatch')) t.push({ to: 'OUT_FOR_DELIVERY', label: 'Mark Out for Delivery', style: 'primary' })
    if (perms.includes('orders:cancel')) t.push({ to: 'CANCELLED', label: 'Cancel Order', style: 'danger' })
  }
  return t
})
...
async function updatePaymentStatus(status) {
  ...
  try {
    ...
    order.value = { ...order.value, paymentStatus: status, paymentNote: updated.paymentNote, paymentVerifiedAt: updated.paymentVerifiedAt, paymentVerifiedBy: updated.paymentVerifiedBy }
...
async function handleTransition(status) {
  if (status === 'OUT_FOR_DELIVERY') {
    ...
    selectedDeliveryPerson.value = deliveryPersons.value[0]?.id ?? null
...
async function confirmDispatch() {
  ...
  await doStatusUpdate('OUT_FOR_DELIVERY')
}

async function doStatusUpdate(status) {
  updating.value = true
...
useHead({ title: `Order #${route.params.id} — Cashier` })
</script>
