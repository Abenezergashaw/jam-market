<template>
  <div class="space-y-6">
    ...
    <template v-else-if="stats">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div class="card p-4 hover:border-zinc-300 hover:shadow-sm transition-all">
          <p class="text-[11px] font-semibold text-zinc-400 uppercase tracking-wider">Total Orders</p>
          <p class="text-2xl font-bold text-zinc-900 mt-1.5">{{ stats?.totalOrders }}</p>
          <p class="text-xs text-zinc-400 mt-0.5">All time</p>
        </div>
        <div class="card p-4 hover:border-zinc-300 hover:shadow-sm transition-all">
          <p class="text-[11px] font-semibold text-zinc-400 uppercase tracking-wider">Today</p>
          <p class="text-2xl font-bold text-zinc-900 mt-1.5">{{ stats?.todayOrders }}</p>
          <p class="text-xs text-zinc-400 mt-0.5">Orders today</p>
        </div>
        <div class="card p-4 hover:border-zinc-300 hover:shadow-sm transition-all">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div class="card p-4 hover:border-zinc-300 hover:shadow-sm transition-all">
          <p class="text-[11px] font-semibold text-zinc-400 uppercase tracking-wider">Products</p>
          <p class="text-2xl font-bold text-zinc-900 mt-1.5">{{ stats?.totalProducts }}</p>
          <p class="text-xs text-zinc-400 mt-0.5">{{ stats?.totalCategories }} categories</p>
        </div>
        <div class="card p-4 hover:border-zinc-300 hover:shadow-sm transition-all">
          <p class="text-[11px] font-semibold text-zinc-400 uppercase tracking-wider">Low Stock</p>
          <p class="text-2xl font-bold mt-1.5" :class="stats?.lowStockCount > 0 ? 'text-amber-500' : 'text-zinc-900'">{{ stats?.lowStockCount }}</p>
          <p class="text-xs text-zinc-400 mt-0.5">Items running low</p>
        </div>
        <div class="card p-4 hover:border-zinc-300 hover:shadow-sm transition-all">
          <p class="text-[11px] font-semibold text-zinc-400 uppercase tracking-wider">Out of Stock</p>
          <p class="text-2xl font-bold mt-1.5" :class="stats?.outOfStockCount > 0 ? 'text-brand-500' : 'text-zinc-900'">{{ stats?.outOfStockCount }}</p>
          <p class="text-xs text-zinc-400 mt-0.5">Need restocking</p>
        </div>
      </div>
      ...
      <div v-if="stats?.recentOrders?.length" class="card overflow-hidden">
        <div class="px-4 py-3 border-b border-zinc-100 flex items-center justify-between">
          ...
          <NuxtLink to="/admin/orders" class="text-xs text-brand-500 hover:text-brand-600 transition-colors">View all</NuxtLink>
        </div>
      ...
      <div v-if="stats?.lowStockProducts?.length" class="card p-5">
        <h2 class="font-semibold text-zinc-800 mb-3 flex items-center gap-2">
          ...
          Low Stock Alert
          <NuxtLink to="/admin/products/low-stock" class="ml-auto text-xs text-brand-500 hover:text-brand-600 transition-colors">Manage stock</NuxtLink>
        </h2>
        <ul class="space-y-1">
          <li
    ...
    <div v-else-if="error" class="card p-5 text-brand-700 text-sm">Failed to load dashboard. Check your connection.</div>
  </div>
</template>
