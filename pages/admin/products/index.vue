<template>
  <div class="space-y-4">
    <div class="flex items-center justify-between gap-3">
      <p class="text-sm text-zinc-500">{{ products?.length ?? 0 }} products</p>
      <NuxtLink to="/admin/products/new" class="btn-primary text-sm py-2">+ Add product</NuxtLink>
    </div>
    ...
    <div v-else-if="!products?.length" class="card p-14 text-center">
      <p class="text-zinc-500 text-sm mb-3">No products yet.</p>
      <NuxtLink to="/admin/products/new" class="text-brand-400 hover:text-brand-300 text-sm transition-colors">Create the first one</NuxtLink>
    </div>
        ...
        <table class="w-full text-sm min-w-[560px]">
          ...
          <tbody class="divide-y divide-zinc-800/60">
            <tr v-for="product in products" :key="product.id" class="hover:bg-zinc-800/30 transition-colors">
              ...
              <td class="px-4 py-3 font-semibold text-zinc-200">${{ Number(product.price).toFixed(2) }}</td>
              <td class="px-4 py-3">
              ...
              <td class="px-4 py-3 text-right">
                <div class="flex items-center justify-end gap-3">
                  <NuxtLink :to="`/admin/products/${product.id}`" class="text-xs text-brand-400 hover:text-brand-300 transition-colors">Edit</NuxtLink>
                  <button class="text-xs text-zinc-600 hover:text-red-400 transition-colors" @click="deleteProduct(product)">Delete</button>
                </div>
              </td>
            </tr>
          </tbody>
